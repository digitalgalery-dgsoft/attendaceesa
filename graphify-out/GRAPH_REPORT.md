# Graph Report - New  (2026-09-25)

## Corpus Check
- Large corpus: 1440 files · ~4,657,000 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder.

## Summary
- 13973 nodes · 33471 edges · 792 communities (273 shown, 519 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 1989 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Community Hubs (Navigation)
- Employee & Identity Management
- Employee & Identity Management
- Subsystem: ReportTemplate
- API & Mobile State Providers
- Odoo ERP & Principal Integration
- Subsystem: rich-editor.js
- Odoo ERP & Principal Integration
- Visit & Itinerary Management
- Reporting & Form Engine
- Employee & Identity Management
- Attendance & Schedule Subsystem
- Attendance & Schedule Subsystem
- Attendance & Schedule Subsystem
- Attendance & Schedule Subsystem
- Attendance & Schedule Subsystem
- Admin Panel Filament Resources
- API & Mobile State Providers
- Subsystem: constructor()
- Attendance & Schedule Subsystem
- Admin Panel Filament Resources
- Subsystem: resolve()
- Flutter Mobile UI Components
- Reporting & Form Engine
- Attendance & Schedule Subsystem
- Attendance & Schedule Subsystem
- Flutter Mobile UI Components
- Flutter Mobile UI Components
- Subsystem: W()
- Subsystem: markdown-editor.js
- Subsystem: draw()
- Attendance & Schedule Subsystem
- Attendance & Schedule Subsystem
- Admin Panel Filament Resources
- Subsystem: e()
- Employee & Identity Management
- Flutter Mobile UI Components
- Attendance & Schedule Subsystem
- Subsystem: y()
- Admin Panel Filament Resources
- Subsystem: constructor()
- Employee & Identity Management
- Attendance & Schedule Subsystem
- Attendance & Schedule Subsystem
- Subsystem: step()
- Subsystem: t()
- Subsystem: constructor()
- Subsystem: s()
- Reporting & Form Engine
- Reporting & Form Engine
- Subsystem: _update()
- Attendance & Schedule Subsystem
- Subsystem: i()
- Subsystem: Ae()
- Attendance & Schedule Subsystem
- Subsystem: dependencies
- Subsystem: __fire()
- Subsystem: _update()
- Visit & Itinerary Management
- Subsystem: file-upload.js
- Subsystem: support.js
- Admin Panel Filament Resources
- Subsystem: flush()
- Subsystem: push()
- Subsystem: facet()
- Reporting & Form Engine
- Odoo ERP & Principal Integration
- Employee & Identity Management
- Subsystem: O()
- Flutter Mobile UI Components
- Flutter Mobile UI Components
- Admin Panel Filament Resources
- Subsystem: getContext()
- Odoo ERP & Principal Integration
- Subsystem: fn()
- Odoo ERP & Principal Integration
- Subsystem: enable()
- Flutter Mobile UI Components
- Subsystem: columns/select.js
- Subsystem: draw()
- Flutter Mobile UI Components
- API & Mobile State Providers
- Reporting & Form Engine
- Subsystem: slice()
- Employee & Identity Management
- Employee & Identity Management
- Admin Panel Filament Resources
- Subsystem: lineAt()
- Subsystem: nodesBetween()
- Subsystem: ir()
- Subsystem: T()
- Attendance & Schedule Subsystem
- Reporting & Form Engine
- Subsystem: reduce()
- Subsystem: of()
- Subsystem: Ew()
- Subsystem: qt()
- API & Mobile State Providers
- Subsystem: lP()
- Attendance & Schedule Subsystem
- Admin Panel Filament Resources
- Subsystem: slider.js
- Admin Panel Filament Resources
- Admin Panel Filament Resources
- Subsystem: att-admindashboard/compos
- Subsystem: Xt()
- Flutter Mobile UI Components
- Subsystem: getDatasetMeta()
- Flutter Mobile UI Components
- Subsystem: on()
- Subsystem: Si()
- Subsystem: d()
- Subsystem: Cn()
- Attendance & Schedule Subsystem
- Attendance & Schedule Subsystem
- Subsystem: n()
- Admin Panel Filament Resources
- Subsystem: getDatasetMeta()
- Employee & Identity Management
- Visit & Itinerary Management
- API & Mobile State Providers
- Attendance & Schedule Subsystem
- Admin Panel Filament Resources
- Subsystem: echo.js
- Subsystem: add()
- Attendance & Schedule Subsystem
- Reporting & Form Engine
- Subsystem: components/select.js
- API & Mobile State Providers
- Subsystem: _updateHiddenPolyCircle()
- Subsystem: updateElements()
- Reporting & Form Engine
- Flutter Mobile UI Components
- Employee & Identity Management
- Reporting & Form Engine
- Subsystem: fn()
- Subsystem: get()
- Subsystem: _a()
- API & Mobile State Providers
- API & Mobile State Providers
- Admin Panel Filament Resources
- Subsystem: TeamUncheckedMonitoring
- Admin Panel Filament Resources
- Employee & Identity Management
- Admin Panel Filament Resources
- Attendance & Schedule Subsystem
- Subsystem: match()
- Employee & Identity Management
- Odoo ERP & Principal Integration
- Subsystem: octane.php
- Reporting & Form Engine
- Employee & Identity Management
- API & Mobile State Providers
- Subsystem: att-admin-v12/package.jso
- Subsystem: oo()
- Admin Panel Filament Resources
- Attendance & Schedule Subsystem
- Employee & Identity Management
- Odoo ERP & Principal Integration
- API & Mobile State Providers
- Employee & Identity Management
- Flutter Mobile UI Components
- Reporting & Form Engine
- API & Mobile State Providers
- Flutter Mobile UI Components
- Subsystem: t()
- Subsystem: color-picker.js
- Subsystem: fn()
- Admin Panel Filament Resources
- Subsystem: TestCase
- Admin Panel Filament Resources
- Attendance & Schedule Subsystem
- API & Mobile State Providers
- Admin Panel Filament Resources
- Subsystem: .prettierrc.json
- Reporting & Form Engine
- Flutter Mobile UI Components
- Attendance & Schedule Subsystem
- Attendance & Schedule Subsystem
- Employee & Identity Management
- Reporting & Form Engine
- Flutter Mobile UI Components
- Subsystem: renderOptions()
- Subsystem: e()
- Employee & Identity Management
- Attendance & Schedule Subsystem
- Subsystem: ResetPasswordMail
- Admin Panel Filament Resources
- Subsystem: date-time-picker.js
- Subsystem: app-calendar.js
- Admin Panel Filament Resources
- Subsystem: Doc: SKILL.md
- Attendance & Schedule Subsystem
- Reporting & Form Engine
- Subsystem: _calcLatLngDistances()
- Flutter Mobile UI Components
- Subsystem: webpack.mix.js
- Employee & Identity Management
- Subsystem: Doc: SKILL.md
- Subsystem: Doc: SKILL.md
- Attendance & Schedule Subsystem
- Subsystem: MessageSent
- Subsystem: Fe()
- Attendance & Schedule Subsystem
- Reporting & Form Engine
- Admin Panel Filament Resources
- Subsystem: Doc: implementation-playb
- Subsystem: Doc: SKILL.md
- Subsystem: Doc: SKILL.md
- Subsystem: Doc: PANDUAN_MIGRASI_SWIT
- Odoo ERP & Principal Integration
- Reporting & Form Engine
- Visit & Itinerary Management
- Flutter Mobile UI Components
- Employee & Identity Management
- Subsystem: PermitStatusUpdated
- Subsystem: att-admin-v12/bootstrap/a
- Subsystem: scripts
- Admin Panel Filament Resources
- Employee & Identity Management
- Admin Panel Filament Resources
- Admin Panel Filament Resources
- Subsystem: require-dev
- Subsystem: jt()
- API & Mobile State Providers
- Attendance & Schedule Subsystem
- Flutter Mobile UI Components
- Employee & Identity Management
- Subsystem: scripts
- Reporting & Form Engine
- Subsystem: Doc: README.md
- Subsystem: Doc: README.md
- Attendance & Schedule Subsystem
- Subsystem: os
- API & Mobile State Providers
- Odoo ERP & Principal Integration
- Subsystem: PHPUnit\Framework\TestCas
- Subsystem: app-kanban.js
- Subsystem: payslip.dart
- Employee & Identity Management
- Admin Panel Filament Resources
- Subsystem: Doc: implementation-playb
- Subsystem: Doc: SKILL.md
- Subsystem: Doc: SKILL.md
- Attendance & Schedule Subsystem
- Odoo ERP & Principal Integration
- Subsystem: LiveChat.php
- Employee & Identity Management
- Subsystem: att-admin-v12/config/logg
- Subsystem: components/actions.js
- Attendance & Schedule Subsystem
- Subsystem: Handler.php
- Employee & Identity Management
- Employee & Identity Management
- Subsystem: sys
- Admin Panel Filament Resources
- API & Mobile State Providers
- Employee & Identity Management
- Admin Panel Filament Resources
- Attendance & Schedule Subsystem
- API & Mobile State Providers
- Subsystem: psr-4
- Odoo ERP & Principal Integration
- Subsystem: att-admin-v12/deploy.sh
- Employee & Identity Management
- Reporting & Form Engine
- Employee & Identity Management
- Subsystem: overrides
- Subsystem: resolutions
- Subsystem: contentNavbarLayout.blade
- Subsystem: horizontalLayout.blade.ph
- Subsystem: deploy.sh
- Attendance & Schedule Subsystem
- API & Mobile State Providers
- Admin Panel Filament Resources
- Attendance & Schedule Subsystem
- Employee & Identity Management
- API & Mobile State Providers
- Subsystem: app-calendar-events.js
- Reporting & Form Engine
- Subsystem: maps-leaflet.js
- Subsystem: app-invoice-edit.blade.ph
- Flutter Mobile UI Components
- Employee & Identity Management
- Subsystem: commonMaster.blade.php
- Flutter Mobile UI Components
- Attendance & Schedule Subsystem
- Reporting & Form Engine
- Subsystem: att-admin-v12/artisan
- Subsystem: extra
- Subsystem: pt()
- Reporting & Form Engine
- Subsystem: AccessPermission.php
- Subsystem: InvoiceAdd.php
- Subsystem: InvoiceList.php
- Subsystem: InvoicePrint.php
- Employee & Identity Management
- Employee & Identity Management
- Employee & Identity Management
- Subsystem: ForgotPasswordCover.php
- Employee & Identity Management
- Subsystem: RegisterBasic.php
- Subsystem: RegisterCover.php
- Subsystem: TwoStepsCover.php
- Subsystem: VerifyEmailBasic.php
- Subsystem: VerifyEmailCover.php
- Flutter Mobile UI Components
- Subsystem: ApexCharts.php
- Subsystem: Analytics.php
- Subsystem: Crm.php
- Subsystem: DragAndDrop.php
- Subsystem: Misc.php
- Subsystem: StarRatings.php
- Subsystem: Tour.php
- Flutter Mobile UI Components
- Subsystem: InputGroups.php
- Subsystem: Picker.php
- Subsystem: Selects.php
- Subsystem: Sliders.php
- Subsystem: Switches.php
- Admin Panel Filament Resources
- Subsystem: Numbered.php
- Admin Panel Filament Resources
- Subsystem: Blank.php
- Subsystem: Container.php
- Subsystem: ContentNavSidebar.php
- Subsystem: WithoutMenu.php
- Subsystem: AccountSettingsAccount.ph
- Subsystem: AccountSettingsNotificati
- Subsystem: AccountSettingsSecurity.p
- Subsystem: Faq.php
- Subsystem: HelpCenterCategories.php
- Subsystem: MiscError.php
- Employee & Identity Management
- Subsystem: Pricing.php
- Employee & Identity Management
- Employee & Identity Management
- Subsystem: Basic.php
- Admin Panel Filament Resources
- Subsystem: Accordion.php
- Subsystem: Collapse.php
- Subsystem: Dropdowns.php
- Subsystem: Offcanvas.php
- Subsystem: TabsPills.php
- Subsystem: Toasts.php
- Subsystem: TooltipsPopovers.php
- Subsystem: Typography.php
- Subsystem: PropertyListing.php
- API & Mobile State Providers
- API & Mobile State Providers
- API & Mobile State Providers
- API & Mobile State Providers
- Employee & Identity Management
- Subsystem: initSelect2()
- Subsystem: app-access-permission.bla
- Subsystem: app-invoice-add.blade.php
- Employee & Identity Management
- Employee & Identity Management
- Employee & Identity Management
- Subsystem: verticalMenu.blade.php
- Subsystem: Doc: implementation-playb
- Subsystem: Doc: README.md
- Attendance & Schedule Subsystem
- Subsystem: admin.server_monitoring
- Odoo ERP & Principal Integration
- Reporting & Form Engine
- Subsystem: att-admin-v12/deploy-all-
- Employee & Identity Management
- Attendance & Schedule Subsystem
- Subsystem: att-admin-v12/setup-ssh-k
- Subsystem: test.dart
- Subsystem: att-admindashboard/config
- Subsystem: 7.4/start-container
- Subsystem: 8.0/start-container
- Subsystem: 8.1/start-container
- Subsystem: babel
- Employee & Identity Management
- Employee & Identity Management
- Subsystem: app-invoice-print.blade.p
- Employee & Identity Management
- Employee & Identity Management
- Employee & Identity Management
- Employee & Identity Management
- Employee & Identity Management
- Employee & Identity Management
- Employee & Identity Management
- Employee & Identity Management
- Employee & Identity Management
- Employee & Identity Management
- Employee & Identity Management
- Employee & Identity Management
- Flutter Mobile UI Components
- Flutter Mobile UI Components
- Attendance & Schedule Subsystem
- Subsystem: horizontalMenu.blade.php
- Subsystem: submenu.blade.php
- Subsystem: navbar.blade.php
- Subsystem: translations.dart
- Subsystem: deploy-all-production.sh
- Subsystem: Doc: deployment_pipeline.
- Subsystem: Doc: graphify.md
- Subsystem: Doc: versioning.md
- Subsystem: Doc: graphify.md
- Subsystem: Doc: DRAFT_EMAIL_PENGAJUA
- Subsystem: Doc: DRAFT_EMAIL_SINGKAT_
- API & Mobile State Providers
- Subsystem: Doc: struktur_menu.md
- Subsystem: setup-ssh-keys.sh
- Subsystem: @arina
- Subsystem: Doc: .cpanel.yml
- Subsystem: Doc: Meeting Reporint Win
- Subsystem: Doc: index.html
- Subsystem: Doc: .cpanel.yml
- Subsystem: Doc: deploy.yml
- Subsystem: Doc: deploy_log.html
- Subsystem: Doc: diag3.html
- Subsystem: Doc: diag4.html
- Subsystem: Doc: diag5.html
- Subsystem: Doc: diag6.html
- Subsystem: Doc: diag_output.html
- Subsystem: Doc: diag_output2.html
- Subsystem: Doc: dulux_prod.html
- Subsystem: Doc: import_exec_res.html
- Subsystem: Doc: import_exec_res2.htm
- Subsystem: Doc: import_exec_res3.htm
- Subsystem: Doc: import_result_amk.ht
- Subsystem: Doc: import_verify.html
- Subsystem: Doc: robots.txt
- Subsystem: Doc: raw_import_out.html
- Subsystem: Doc: tinker_res.html
- Subsystem: Doc: .styleci.yml
- Subsystem: Doc: robots.txt
- Subsystem: Doc: build_log.txt
- Subsystem: Doc: build_log_2.txt
- Subsystem: Doc: debug_fcm_response.t
- Subsystem: Doc: deploy_out.txt
- Subsystem: Doc: deploy_prod_amk.html
- Subsystem: Doc: import_amk_output.ht
- Attendance & Schedule Subsystem
- Odoo ERP & Principal Integration
- Flutter Mobile UI Components

## God Nodes (most connected - your core abstractions)
1. `Controller` - 315 edges
2. `ReportTemplate` - 206 edges
3. `Principal` - 191 edges
4. `Employee` - 178 edges
5. `AuthProvider` - 148 edges
6. `constructor()` - 146 edges
7. `update()` - 144 edges
8. `ReportFormField` - 117 edges
9. `WorkLocation` - 98 edges
10. `resolve()` - 98 edges

## Surprising Connections (you probably didn't know these)
- `{closure#7}()` --calls--> `AttendanceRosterMatrixExport`  [INFERRED]
  att-admin-v12/app/Filament/Resources/Attendances/Pages/AttendanceRoster.php → att-admin-v12/app/Exports/AttendanceRosterMatrixExport.php
- `AttendanceController` --inherits--> `Controller`  [EXTRACTED]
  att-admindashboard/app/Http/Controllers/Api/AttendanceController.php → att-admin-v12/app/Http/Controllers/Controller.php
- `AuthController` --inherits--> `Controller`  [EXTRACTED]
  att-admindashboard/app/Http/Controllers/Api/AuthController.php → att-admin-v12/app/Http/Controllers/Controller.php
- `AccessPermission` --inherits--> `Controller`  [EXTRACTED]
  att-admindashboard/app/Http/Controllers/apps/AccessPermission.php → att-admin-v12/app/Http/Controllers/Controller.php
- `InvoiceAdd` --inherits--> `Controller`  [EXTRACTED]
  att-admindashboard/app/Http/Controllers/apps/InvoiceAdd.php → att-admin-v12/app/Http/Controllers/Controller.php

## Import Cycles
- None detected.

## Communities (792 total, 519 thin omitted)

### Community 0 - "Employee & Identity Management"
Cohesion: 0.01
Nodes (238): {closure#1}(), {closure#2}(), {closure#1}(), {closure#2}(), {closure#3}(), {closure#1}(), {closure#2}(), {closure#1}() (+230 more)

### Community 1 - "Employee & Identity Management"
Cohesion: 0.01
Nodes (99): Ac(), addActive(), addCompletion(), addCompletions(), addNamespace(), addNamespaceObject(), Ag(), Ar() (+91 more)

### Community 2 - "Subsystem: ReportTemplate"
Cohesion: 0.02
Nodes (84): CheckDuluxProductsCommand, ImportDuluxDailyMaintenanceCommand, ImportDuluxOfftakeCommand, ImportDuluxOosCommand, ImportDuluxStockCommand, LinkPrincipalsCommand, Principal, {closure#1}() (+76 more)

### Community 3 - "API & Mobile State Providers"
Cohesion: 0.01
Nodes (131): abutsStart(), addControllers(), addPlugins(), addScales(), afterDraw(), Be(), bm(), Bt() (+123 more)

### Community 5 - "Subsystem: rich-editor.js"
Cohesion: 0.01
Nodes (151): $0(), Ab(), ac(), accepts(), addGlobalAttributes(), addHackNode(), addMark(), addPasteRules() (+143 more)

### Community 6 - "Odoo ERP & Principal Integration"
Cohesion: 0.01
Nodes (56): CheckPrincipalsCommand, CleanDuluxWorkLocationsCommand, CreateDuluxDummyEmployeesCommand, DbLocksCommand, FixEmptyLocationCodesCommand, FixOrphanedEmployeeDataCommand, ImportAreasCommand, ImportDuluxAmkStoresCommand (+48 more)

### Community 7 - "Visit & Itinerary Management"
Cohesion: 0.02
Nodes (52): {closure#11}(), {closure#12}(), {closure#5}(), {closure#7}(), {closure#5}(), {closure#6}(), {closure#9}(), EditWorkingGroup (+44 more)

### Community 8 - "Reporting & Form Engine"
Cohesion: 0.01
Nodes (236): DynamicReportingProvider, _activeDiscountType, _addCurrentOosToCart, _addPreviousOosToCart, _address, _allowedRadiusMeter, _autoFillMachineSerial, _autoSelectNextUnsubmittedMachine (+228 more)

### Community 9 - "Employee & Identity Management"
Cohesion: 0.01
Nodes (72): PayslipApiController, Controller, AccessRoles, Calendar, Chat, Email, InvoiceEdit, InvoicePreview (+64 more)

### Community 10 - "Attendance & Schedule Subsystem"
Cohesion: 0.01
Nodes (193): _activeMeeting, _activePermit, _canCheckin, _canVisit, checkAttendanceStatus, _checkinBlockMessage, _error, fetchHistory (+185 more)

### Community 11 - "Attendance & Schedule Subsystem"
Cohesion: 0.02
Nodes (83): AreaForm, {closure#1}(), {closure#2}(), AttendanceForm, {closure#2}(), CompetitorProductForm, {closure#3}(), {closure#1}() (+75 more)

### Community 12 - "Attendance & Schedule Subsystem"
Cohesion: 0.02
Nodes (58): AreasTable, AttendanceBapForm, AttendanceBapsTable, AttendancesTable, BranchForm, BranchesTable, CompanyForm, CompaniesTable (+50 more)

### Community 13 - "Attendance & Schedule Subsystem"
Cohesion: 0.02
Nodes (41): {closure#10}(), {closure#11}(), {closure#12}(), {closure#13}(), {closure#14}(), {closure#15}(), {closure#16}(), {closure#17}() (+33 more)

### Community 14 - "Attendance & Schedule Subsystem"
Cohesion: 0.02
Nodes (59): ListAreas, ListAttendanceBaps, CreateAttendance, ListAttendances, CreateBlastInfo, ListBlastInfos, CreateBranch, ListBranches (+51 more)

### Community 15 - "Admin Panel Filament Resources"
Cohesion: 0.02
Nodes (93): addAndReturn(), addNewRoot(), ae(), applyLimitFilters(), _applyStyleClasses(), bbox(), Br(), Bt() (+85 more)

### Community 16 - "API & Mobile State Providers"
Cohesion: 0.02
Nodes (99): acquireContext(), addControllers(), addPlugins(), addScales(), as(), bl(), bo(), br() (+91 more)

### Community 17 - "Subsystem: constructor()"
Cohesion: 0.03
Nodes (133): add(), addChunk(), addEventListener(), addInfoPane(), addInner(), addWindowListeners(), adjust(), annotation() (+125 more)

### Community 18 - "Attendance & Schedule Subsystem"
Cohesion: 0.02
Nodes (123): address, createState, _currentPosition, dispose, _getCurrentLocation, _getScheduledLocations, id, initialWorkLocationId (+115 more)

### Community 19 - "Admin Panel Filament Resources"
Cohesion: 0.02
Nodes (42): AiSettings, BackedEnum, UnitEnum, ManageSettings, BackedEnum, UnitEnum, {closure#1}(), {closure#2}() (+34 more)

### Community 20 - "Subsystem: resolve()"
Cohesion: 0.06
Nodes (128): ad(), addKeyboardShortcuts(), after(), apply(), Ay(), before(), between(), blockRange() (+120 more)

### Community 21 - "Flutter Mobile UI Components"
Cohesion: 0.03
Nodes (122): _a(), addNodeView(), addOptions(), ak(), c(), d(), atEnd(), b0() (+114 more)

### Community 22 - "Reporting & Form Engine"
Cohesion: 0.02
Nodes (119): AuthProvider, LocaleProvider, ThemeProvider, build, _showPendingReportsAlert, _takeSelfie, _buildMeetingCard, build (+111 more)

### Community 23 - "Attendance & Schedule Subsystem"
Cohesion: 0.03
Nodes (29): App\Filament\Resources\PayslipResource\Pages, App\Filament\Resources\VisitReportResource\Pages, App\Filament\Resources\WorkingGroupResource\Pages, App\Filament\Resources\WorkTargetResource\Pages, {closure#15}(), {closure#16}(), {closure#1}(), {closure#12}() (+21 more)

### Community 24 - "Attendance & Schedule Subsystem"
Cohesion: 0.02
Nodes (109): Animation, AnimationController, AttendanceApp, address, canEdit, fieldLabel, fieldName, fieldType (+101 more)

### Community 25 - "Flutter Mobile UI Components"
Cohesion: 0.03
Nodes (114): addBlockWidget(), addBreak(), addComposition(), addDelimiter(), addInlineWidget(), addLine(), addLineStart(), addLineStartIfNotCovered() (+106 more)

### Community 26 - "Flutter Mobile UI Components"
Cohesion: 0.03
Nodes (94): DashboardProvider, NotificationProvider, OvertimeProvider, PayslipProvider, _loadTheme, _themeMode, toggleTheme, build (+86 more)

### Community 27 - "Subsystem: W()"
Cohesion: 0.04
Nodes (104): aa(), aO(), atLastNode(), au(), b1(), Bg(), ch(), charCategorizer() (+96 more)

### Community 28 - "Subsystem: markdown-editor.js"
Cohesion: 0.04
Nodes (93): ad(), af(), An(), ao(), Ba(), bc(), bf(), Bi() (+85 more)

### Community 29 - "Subsystem: draw()"
Cohesion: 0.04
Nodes (103): acquireContext(), adjustHitBoxes(), Ao(), aspectRatio(), B(), bh(), buildTicks(), calculateLabelRotation() (+95 more)

### Community 30 - "Attendance & Schedule Subsystem"
Cohesion: 0.03
Nodes (33): AreaResource, CreateArea, EditArea, EditAttendanceBap, EditAttendance, EditBlastInfo, EditBranch, EditCompany (+25 more)

### Community 31 - "Attendance & Schedule Subsystem"
Cohesion: 0.02
Nodes (98): AttendanceProvider, ItineraryProvider, AddItineraryScreen, _AddItineraryScreenState, _agendaController, _buildSearchableSelectField, createState, _fetchData (+90 more)

### Community 32 - "Admin Panel Filament Resources"
Cohesion: 0.04
Nodes (95): Fe(), ac(), ae(), after(), ag(), Al(), Am(), before() (+87 more)

### Community 33 - "Subsystem: e()"
Cohesion: 0.05
Nodes (95): addCommands(), addProseMirrorPlugins(), Ap(), atStart(), bk(), bp(), br(), c0() (+87 more)

### Community 34 - "Employee & Identity Management"
Cohesion: 0.03
Nodes (90): AuthWrapper, _AuthWrapperState, build, createState, didChangeAppLifecycleState, dispose, _hasSeenOnboarding, _initFuture (+82 more)

### Community 35 - "Flutter Mobile UI Components"
Cohesion: 0.02
Nodes (90): MeetingDetailModel, MeetingModel, build, _buildCaseOption, _buildLiveChatView, _buildNikFormView, _buildProfileRow, _buildTicketCard (+82 more)

### Community 36 - "Attendance & Schedule Subsystem"
Cohesion: 0.07
Nodes (11): ServerGatewayController, TenantAuthController, Carbon, PrincipalPortalController, Setting, GET /(), Request, SortDirection (+3 more)

### Community 37 - "Subsystem: y()"
Cohesion: 0.07
Nodes (65): Dg(), Ig(), Se(), Ud(), al(), at(), Be(), Cr() (+57 more)

### Community 38 - "Admin Panel Filament Resources"
Cohesion: 0.02
Nodes (91): browserslist, name, private, version, animate.css, aos, apexcharts-clevision, autosize (+83 more)

### Community 39 - "Subsystem: constructor()"
Cohesion: 0.03
Nodes (90): add(), addAttributes(), addExtensions(), addNode(), ag(), applyInitialSize(), Cc(), cg() (+82 more)

### Community 40 - "Employee & Identity Management"
Cohesion: 0.04
Nodes (87): acceptToken(), addChild(), addGaps(), addLeafElement(), addNode(), advance(), allows(), ATXHeading() (+79 more)

### Community 41 - "Attendance & Schedule Subsystem"
Cohesion: 0.03
Nodes (28): ShiftImporter, {closure#11}(), {closure#12}(), {closure#14}(), {closure#15}(), {closure#16}(), {closure#19}(), {closure#23}() (+20 more)

### Community 42 - "Attendance & Schedule Subsystem"
Cohesion: 0.03
Nodes (84): PermitProvider, SalesProvider, AttendanceLocationScreen, _AttendanceLocationScreenState, ChatScreen, _ChatScreenState, HelpdeskChatScreen, _HelpdeskChatScreenState (+76 more)

### Community 43 - "Subsystem: step()"
Cohesion: 0.05
Nodes (84): addNodeMark(), allowedMarks(), allowsMarks(), am(), append(), as(), au(), bc() (+76 more)

### Community 44 - "Subsystem: t()"
Cohesion: 0.04
Nodes (84): Ai(), ao(), Ar(), as(), Ba(), bi(), bn(), ci() (+76 more)

### Community 45 - "Subsystem: constructor()"
Cohesion: 0.04
Nodes (83): $a(), alpha(), apply(), Bc(), bg(), ca(), chartOptionScopes(), Ci() (+75 more)

### Community 46 - "Subsystem: s()"
Cohesion: 0.04
Nodes (79): Eo(), ac(), addEventListener(), al(), average(), bindResponsiveEvents(), bs(), cc() (+71 more)

### Community 47 - "Reporting & Form Engine"
Cohesion: 0.03
Nodes (77): applyDuluxSequence, assignedEmployees, assignedPositions, barcode, brand, _calcIsWithinMonthlyRange, category, code (+69 more)

### Community 48 - "Reporting & Form Engine"
Cohesion: 0.04
Nodes (16): OdooSyncReport, BackedEnum, UnitEnum, ActiveEmployeesHourlyChartWidget, AttendanceChartWidget, MandaysChartWidget, ManPowerChartWidget, TurnOverChartWidget (+8 more)

### Community 49 - "Subsystem: _update()"
Cohesion: 0.05
Nodes (77): addBox(), addElements(), addEventListener(), afterBuildTicks(), afterCalculateLabelRotation(), afterDataLimits(), afterFit(), afterSetDimensions() (+69 more)

### Community 50 - "Attendance & Schedule Subsystem"
Cohesion: 0.04
Nodes (40): {closure#10}(), {closure#2}(), {closure#3}(), {closure#4}(), {closure#5}(), {closure#6}(), {closure#7}(), BranchResource (+32 more)

### Community 51 - "Subsystem: i()"
Cohesion: 0.04
Nodes (73): al(), AQ(), AX(), be(), between(), a(), blockTiles(), combine() (+65 more)

### Community 52 - "Subsystem: Ae()"
Cohesion: 0.09
Nodes (72): _a(), Ae(), ar(), as(), me(), ue(), u(), ci() (+64 more)

### Community 53 - "Attendance & Schedule Subsystem"
Cohesion: 0.03
Nodes (23): AttendanceBapResource, UnitEnum, CreateAttendanceBap, ViewAttendanceBap, ViewLeaveRequest, LocationRequestResource, Builder, UnitEnum (+15 more)

### Community 54 - "Subsystem: dependencies"
Cohesion: 0.03
Nodes (72): dependencies, animate.css, aos, apexcharts-clevision, autosize, block-ui, bloodhound-js, bootstrap (+64 more)

### Community 55 - "Subsystem: __fire()"
Cohesion: 0.05
Nodes (71): _addMarker(), calcMiddleLatLng(), _change(), _checkMarkerAllowedToDrag(), _createCenterMarker(), _createMiddleMarker(), _createOuterMarker(), _createVertex() (+63 more)

### Community 56 - "Subsystem: _update()"
Cohesion: 0.05
Nodes (70): afterBuildTicks(), afterCalculateLabelRotation(), afterDataLimits(), afterDraw(), afterFit(), afterSetDimensions(), afterTickToLabelConversion(), afterUpdate() (+62 more)

### Community 57 - "Visit & Itinerary Management"
Cohesion: 0.03
Nodes (14): {closure#1}(), {closure#3}(), {closure#1}(), {closure#3}(), up(), PermissionsSeeder, Illuminate\Database\Console\Seeds\WithoutModelEvents, Illuminate\Foundation\Inspiring (+6 more)

### Community 58 - "Subsystem: file-upload.js"
Cohesion: 0.05
Nodes (50): am(), Ap(), be(), bi(), Bp(), c(), clickPercent(), cm() (+42 more)

### Community 59 - "Subsystem: support.js"
Cohesion: 0.05
Nodes (51): apply(), as(), At(), bo(), bs(), close(), closeQuietly(), co() (+43 more)

### Community 60 - "Admin Panel Filament Resources"
Cohesion: 0.04
Nodes (68): Cl(), clone(), create(), Ct(), dtFormatter(), Ea(), Ec(), eras() (+60 more)

### Community 61 - "Subsystem: flush()"
Cohesion: 0.05
Nodes (65): Ar(), Bd(), disconnectSelection(), dl(), domAfterPos(), domAtPos(), domFromPos(), domSelection() (+57 more)

### Community 62 - "Subsystem: push()"
Cohesion: 0.08
Nodes (63): _a(), addInitHooks(), a(), c(), h(), o(), p(), r() (+55 more)

### Community 63 - "Subsystem: facet()"
Cohesion: 0.05
Nodes (64): accept(), activateHover(), Ah(), applyTransaction(), asSingle(), baseDirAt(), bidiIn(), bidiSpansAt() (+56 more)

### Community 64 - "Reporting & Form Engine"
Cohesion: 0.03
Nodes (60): _allCompetitorProducts, _applyCompetitorData, _applyDefaultCompetitorMatrix, checkCompliance, _competitorBrands, _competitorSubbrandsByBrand, _cutoffInfo, _defaultArea (+52 more)

### Community 65 - "Odoo ERP & Principal Integration"
Cohesion: 0.03
Nodes (59): AndroidFlutterLocalNotificationsPlugin, AndroidInitializationSettings, AndroidNotificationChannel, @pragma, cronInterval, cronTimer, distanceMeters, _getCurrentPosition (+51 more)

### Community 66 - "Employee & Identity Management"
Cohesion: 0.05
Nodes (16): App\Models\Permit, {closure#3}(), User, up(), UserManagement, User, Filament\Models\Contracts\FilamentUser, Google\Client (+8 more)

### Community 67 - "Subsystem: O()"
Cohesion: 0.14
Nodes (54): b(), $c(), X(), ca(), D(), _e(), f(), se() (+46 more)

### Community 68 - "Flutter Mobile UI Components"
Cohesion: 0.04
Nodes (56): build, _cameraController, _cameraIndex, _cameras, _canProcess, _captureAndReturn, _concatenatePlanes, createState (+48 more)

### Community 69 - "Flutter Mobile UI Components"
Cohesion: 0.04
Nodes (21): App\Filament\Resources\BlastInfoResource\Pages, App\Filament\Resources\LeaveRequestResource\Pages, BlastInfoResource, {closure#3}(), {closure#4}(), Schema, UnitEnum, {closure#1}() (+13 more)

### Community 70 - "Admin Panel Filament Resources"
Cohesion: 0.05
Nodes (57): afterAutoSkip(), au(), br(), buildLookupTable(), _calculateBarIndexPixels(), _calculateBarValuePixels(), calculateCircumference(), _circumference() (+49 more)

### Community 71 - "Subsystem: getContext()"
Cohesion: 0.06
Nodes (57): bh(), buildTicks(), calculateLabelRotation(), _calculatePadding(), cn(), _computeAngle(), _computeGridLineItems(), _computeLabelItems() (+49 more)

### Community 72 - "Odoo ERP & Principal Integration"
Cohesion: 0.06
Nodes (13): Login, {closure#3}(), {closure#4}(), {closure#5}(), OdooSync, BackedEnum, UnitEnum, OdooSyncService (+5 more)

### Community 73 - "Subsystem: fn()"
Cohesion: 0.07
Nodes (54): addInputRules(), Ah(), at(), Ax(), Ba(), bx(), c(), Ch() (+46 more)

### Community 74 - "Odoo ERP & Principal Integration"
Cohesion: 0.06
Nodes (16): BranchImporter, CompanyImporter, DepartmentImporter, EmployeeImporter, EmployeeScheduleImporter, PositionImporter, PrincipalImporter, WorkLocationImporter (+8 more)

### Community 75 - "Subsystem: enable()"
Cohesion: 0.07
Nodes (54): add(), _addDrawnLayerProp(), i(), _applyFocus(), _autoResize(), _cleanupSnapping(), _createHintLine(), _createMarker() (+46 more)

### Community 76 - "Flutter Mobile UI Components"
Cohesion: 0.06
Nodes (31): actions(), button(), c(), close(), configureAnimations(), configureTransitions(), constructor(), danger() (+23 more)

### Community 77 - "Subsystem: columns/select.js"
Cohesion: 0.08
Nodes (43): Ae(), Ai(), An(), applyDisabledState(), be(), Bt(), De(), disable() (+35 more)

### Community 78 - "Subsystem: draw()"
Cohesion: 0.07
Nodes (53): We(), adjustHitBoxes(), At(), beforeDatasetDraw(), beforeDatasetsDraw(), beforeDraw(), bi(), clear() (+45 more)

### Community 79 - "Flutter Mobile UI Components"
Cohesion: 0.07
Nodes (52): applyGlobalOptions(), blur(), _defineButtons(), disableDraw(), disableGlobalCutMode(), disableGlobalDragMode(), disableGlobalEditMode(), disableGlobalRemovalMode() (+44 more)

### Community 80 - "API & Mobile State Providers"
Cohesion: 0.06
Nodes (52): aa(), As(), Ba(), buildOrUpdateControllers(), buildOrUpdateElements(), cd(), _dataCheck(), _destroy() (+44 more)

### Community 81 - "Reporting & Form Engine"
Cohesion: 0.04
Nodes (3): {closure#11}(), {closure#26}(), {closure#34}()

### Community 82 - "Subsystem: slice()"
Cohesion: 0.05
Nodes (51): a$(), addChanges(), addSelection(), composeDesc(), decompose(), decomposeLeft(), decomposeRight(), defineModifier() (+43 more)

### Community 83 - "Employee & Identity Management"
Cohesion: 0.04
Nodes (48): build, message, onRetry, SecurityWarningScreen, title, _auth, authenticate, BiometricService (+40 more)

### Community 84 - "Employee & Identity Management"
Cohesion: 0.04
Nodes (49): avatar, completedCount, department, durationSeconds, employeeId, employeeNo, endTime, formattedDuration (+41 more)

### Community 85 - "Admin Panel Filament Resources"
Cohesion: 0.13
Nodes (46): A(), ae(), B(), be(), C(), ce(), E(), ee() (+38 more)

### Community 86 - "Subsystem: lineAt()"
Cohesion: 0.06
Nodes (45): addBlock(), addLineDeco(), applyChanges(), balanced(), baseIndent(), baseIndentFor(), bidiSpans(), blankContent() (+37 more)

### Community 87 - "Subsystem: nodesBetween()"
Cohesion: 0.07
Nodes (48): addAll(), addDOM(), addElement(), addElementByRule(), addStoredMark(), addTextNode(), addToSet(), allowsMarkType() (+40 more)

### Community 88 - "Subsystem: ir()"
Cohesion: 0.11
Nodes (46): Ft(), ir(), at(), be(), ce(), Ct(), de(), Dt() (+38 more)

### Community 89 - "Subsystem: T()"
Cohesion: 0.06
Nodes (48): _a(), aa(), ae(), alpha(), ba(), Bt(), ca(), createResolver() (+40 more)

### Community 90 - "Attendance & Schedule Subsystem"
Cohesion: 0.09
Nodes (15): AttendanceRosterMatrixExport, EmployeeScheduleMatrixTemplateExport, EmployeeScheduleRangeTemplateExport, EmployeeScheduleTemplateExport, MandaysExport, ManPowerExport, TeamUncheckedExport, TurnOverExport (+7 more)

### Community 91 - "Reporting & Form Engine"
Cohesion: 0.09
Nodes (10): LocationRequestApiController, ReportingApiController, SettingSyncController, TemplateSyncController, {closure#1}(), ReportSubmission, GoogleMapsService, SettingSyncService (+2 more)

### Community 92 - "Subsystem: reduce()"
Cohesion: 0.08
Nodes (46): addActions(), advanceFully(), advanceStack(), allActions(), c0(), canShift(), close(), deadEnd() (+38 more)

### Community 93 - "Subsystem: of()"
Cohesion: 0.05
Nodes (45): attrs(), baseTheme(), bi(), bu(), cc(), cO(), define(), domEventHandlers() (+37 more)

### Community 94 - "Subsystem: Ew()"
Cohesion: 0.09
Nodes (45): Ac(), ai(), bl(), cd(), ee(), ce(), Dc(), df() (+37 more)

### Community 95 - "Subsystem: qt()"
Cohesion: 0.10
Nodes (46): ae(), q(), B(), Be(), Ft(), He(), Jt(), Me() (+38 more)

### Community 96 - "API & Mobile State Providers"
Cohesion: 0.04
Nodes (43): formatCurrentTime, formatDateTime, isEnglish, isIndonesian, _keyLanguage, _keyTimezone, _languageCode, languageDisplayName (+35 more)

### Community 97 - "Subsystem: lP()"
Cohesion: 0.07
Nodes (45): addElement(), addToSet(), changeByRange(), changes(), childString(), dd(), fd(), ff() (+37 more)

### Community 98 - "Attendance & Schedule Subsystem"
Cohesion: 0.07
Nodes (18): CheckIfInstalled, CheckInstalled, IdentifyTenantSubdomain, RedirectIfInstalled, SecurityHeadersMiddleware, SmartGatewayRelayMiddleware, JsonResponse, Response (+10 more)

### Community 99 - "Admin Panel Filament Resources"
Cohesion: 0.07
Nodes (42): addMaps(), addStep(), addTransform(), appendMap(), appendMapping(), appendMappingInverted(), bl(), compress() (+34 more)

### Community 100 - "Subsystem: slider.js"
Cohesion: 0.09
Nodes (40): tn(), We(), Ae(), ar(), Be(), Bt(), Ce(), De() (+32 more)

### Community 101 - "Admin Panel Filament Resources"
Cohesion: 0.07
Nodes (43): average(), bi(), bu(), Cs(), dataset(), ef(), eu(), Ft() (+35 more)

### Community 102 - "Admin Panel Filament Resources"
Cohesion: 0.08
Nodes (43): buildOrUpdateScales(), D(), dc(), determineDataLimits(), dh(), diff(), En(), endOf() (+35 more)

### Community 103 - "Subsystem: att-admindashboard/compos"
Cohesion: 0.05
Nodes (42): autoload, autoload-dev, psr-4, files, psr-4, config, optimize-autoloader, preferred-install (+34 more)

### Community 104 - "Subsystem: Xt()"
Cohesion: 0.13
Nodes (42): At(), b(), bi(), bn(), Ce(), ci(), cn(), ct() (+34 more)

### Community 105 - "Flutter Mobile UI Components"
Cohesion: 0.07
Nodes (41): activeForPoint(), as(), boundChange(), commit(), compare(), comparePoint(), compareRange(), createSet() (+33 more)

### Community 106 - "Subsystem: getDatasetMeta()"
Cohesion: 0.06
Nodes (41): afterDatasetsUpdate(), An(), ar(), beforeDatasetDraw(), beforeDatasetsDraw(), beforeDraw(), _d(), datasetElementScopeKeys() (+33 more)

### Community 107 - "Flutter Mobile UI Components"
Cohesion: 0.12
Nodes (40): addBadgesForSelectedOptions(), addSingleBadge(), addSingleSelectionDisplay(), closeDropdown(), constructor(), createBadgeElement(), createOptionElement(), createRemoveButton() (+32 more)

### Community 108 - "Subsystem: on()"
Cohesion: 0.08
Nodes (39): addDraggingClass(), _addMarkerEvents(), _addTouchEvents(), applyOptions(), _assignEvents(), _createSnapList(), disableLayerDrag(), _disableSnapping() (+31 more)

### Community 109 - "Subsystem: Si()"
Cohesion: 0.14
Nodes (39): At(), bi(), bn(), ci(), cn(), ct(), di(), dn() (+31 more)

### Community 110 - "Subsystem: d()"
Cohesion: 0.14
Nodes (36): Ai(), ar(), c(), d(), destroy(), Do(), f(), Fa() (+28 more)

### Community 111 - "Subsystem: Cn()"
Cohesion: 0.16
Nodes (36): Cn(), b(), Ce(), De(), dn(), _e(), F(), Fe() (+28 more)

### Community 112 - "Attendance & Schedule Subsystem"
Cohesion: 0.09
Nodes (15): ShiftTemplateExport, VisitScheduleTemplateExport, {closure#2}(), Maatwebsite\Excel\Concerns\WithColumnFormatting, PhpOffice\PhpSpreadsheet\Cell\Cell, PhpOffice\PhpSpreadsheet\Cell\Coordinate, PhpOffice\PhpSpreadsheet\Cell\DataType, PhpOffice\PhpSpreadsheet\Cell\DefaultValueBinder (+7 more)

### Community 113 - "Attendance & Schedule Subsystem"
Cohesion: 0.07
Nodes (12): AttendanceResource, Builder, UnitEnum, ViewTrackingHistory, MeetingResource, UnitEnum, CreateMeeting, ListMeetings (+4 more)

### Community 114 - "Subsystem: n()"
Cohesion: 0.09
Nodes (31): Aa(), br(), cf(), co(), da(), Dn(), ed(), Eo() (+23 more)

### Community 115 - "Admin Panel Filament Resources"
Cohesion: 0.08
Nodes (37): addAll(), _adjustAllMarkers(), _adjustRectangleForMarkerMove(), an(), clone(), enableRotate(), _findCorners(), _fireRotation() (+29 more)

### Community 116 - "Subsystem: getDatasetMeta()"
Cohesion: 0.08
Nodes (37): themeClasses(), afterDatasetsUpdate(), bc(), beforeLayout(), fc(), gc(), generateLabels(), getDatasetMeta() (+29 more)

### Community 117 - "Employee & Identity Management"
Cohesion: 0.08
Nodes (37): af(), at(), Bf(), determineDataLimits(), Fa(), getAllParsedValues(), getDataTimestamps(), _getLabelBounds() (+29 more)

### Community 118 - "Visit & Itinerary Management"
Cohesion: 0.06
Nodes (6): App\Http\Controllers\Api\EmployeeController, AuthController, BlastInfoController, ItineraryController, NotificationController, GET /user()

### Community 119 - "API & Mobile State Providers"
Cohesion: 0.06
Nodes (22): Dashboard, AdminPanelProvider, EncryptCookies, Filament\Http\Middleware\Authenticate, Filament\Http\Middleware\AuthenticateSession, Filament\Http\Middleware\DisableBladeIconComponents, Filament\Http\Middleware\DispatchServingFilamentEvent, Filament\Pages\Dashboard (+14 more)

### Community 120 - "Attendance & Schedule Subsystem"
Cohesion: 0.07
Nodes (36): active(), apply(), b0(), bd(), Bh(), checkAsyncSchedule(), clearDelayedAndroidKey(), delayAndroidKey() (+28 more)

### Community 121 - "Admin Panel Filament Resources"
Cohesion: 0.07
Nodes (36): ad(), applyStack(), bd(), beforeLayout(), cf(), Do(), Dt(), first() (+28 more)

### Community 122 - "Subsystem: echo.js"
Cohesion: 0.08
Nodes (19): ar(), Be(), cr(), d(), ei(), f(), ii(), le() (+11 more)

### Community 123 - "Subsystem: add()"
Cohesion: 0.08
Nodes (35): active(), add(), _animateOptions(), beforeUpdate(), _cachedScopes(), cancel(), ci(), _createAnimations() (+27 more)

### Community 124 - "Attendance & Schedule Subsystem"
Cohesion: 0.06
Nodes (34): BapScreen, _BapScreenState, build, _buildFormTab, _buildHistoryTab, _buildSectionHeader, _buildTabButton, _categories (+26 more)

### Community 125 - "Reporting & Form Engine"
Cohesion: 0.09
Nodes (33): create_user(), delete_user(), ErrorDetail, ErrorResponse, get_user(), http_exception_handler(), list_users(), PaginatedResponse (+25 more)

### Community 126 - "Subsystem: components/select.js"
Cohesion: 0.09
Nodes (24): A(), An(), applyDisabledState(), be(), Bt(), disable(), e(), ei() (+16 more)

### Community 127 - "API & Mobile State Providers"
Cohesion: 0.06
Nodes (4): {closure#1}(), {closure#3}(), Illuminate\Support\Str, Pdo\Mysql

### Community 128 - "Subsystem: _updateHiddenPolyCircle()"
Cohesion: 0.09
Nodes (33): e(), circleToPolygon(), di(), _distanceCalculation(), fe(), _fireCenterPlaced(), Fr(), _getLatLngOnCircle() (+25 more)

### Community 129 - "Subsystem: updateElements()"
Cohesion: 0.10
Nodes (33): Ao(), applyStack(), _calculateBarIndexPixels(), _calculateBarValuePixels(), calculateCircumference(), _circumference(), countVisibleElements(), _getAxis() (+25 more)

### Community 130 - "Reporting & Form Engine"
Cohesion: 0.06
Nodes (31): BlastInfo, content, departmentId, endDate, fromJson, id, startDate, targetType (+23 more)

### Community 131 - "Flutter Mobile UI Components"
Cohesion: 0.08
Nodes (13): App\Filament\Resources\ExtraHourResource\Pages, {closure#10}(), {closure#11}(), {closure#13}(), {closure#14}(), {closure#16}(), {closure#5}(), {closure#7}() (+5 more)

### Community 132 - "Employee & Identity Management"
Cohesion: 0.06
Nodes (5): {closure#4}(), GET /admin/impersonate-start/{user}(), GET /admin/impersonate/{user}(), User, Illuminate\Support\Facades\Route

### Community 133 - "Reporting & Form Engine"
Cohesion: 0.35
Nodes (3): WingsMbrExportService, PhpOffice\PhpSpreadsheet\Worksheet\Worksheet, Symfony\Component\HttpFoundation\StreamedResponse

### Community 134 - "Subsystem: fn()"
Cohesion: 0.14
Nodes (31): ca(), Cr(), Dn(), e(), fn(), x(), Gi(), ia() (+23 more)

### Community 135 - "Subsystem: get()"
Cohesion: 0.09
Nodes (31): active(), add(), _animateOptions(), bo(), bs(), _cachedScopes(), Ch(), _createAnimations() (+23 more)

### Community 136 - "Subsystem: _a()"
Cohesion: 0.18
Nodes (30): _a(), aa(), ba(), br(), Bt(), ct(), Da(), ei() (+22 more)

### Community 137 - "API & Mobile State Providers"
Cohesion: 0.09
Nodes (29): addElements(), buildOrUpdateControllers(), buildOrUpdateElements(), Ce(), _createItems(), _dataCheck(), datasetElementScopeKeys(), _destroy() (+21 more)

### Community 138 - "API & Mobile State Providers"
Cohesion: 0.09
Nodes (13): AppServiceProvider, {closure#1}(), {closure#2}(), {closure#3}(), {closure#4}(), {closure#5}(), Blueprint, Request (+5 more)

### Community 139 - "Admin Panel Filament Resources"
Cohesion: 0.07
Nodes (28): Doc: implementation-playbook.md, Bash - safer, Bash Defensive Patterns Implementation Playbook, Core Defensive Principles, Correct - safe, Correctly determine script directory, Create temporary directory, Create temporary files in directory (+20 more)

### Community 140 - "Subsystem: TeamUncheckedMonitoring"
Cohesion: 0.09
Nodes (5): {closure#1}(), BackedEnum, UnitEnum, Width, TeamUncheckedMonitoring

### Community 141 - "Admin Panel Filament Resources"
Cohesion: 0.10
Nodes (11): {closure#10}(), {closure#11}(), {closure#12}(), {closure#13}(), {closure#14}(), {closure#15}(), {closure#3}(), {closure#5}() (+3 more)

### Community 142 - "Employee & Identity Management"
Cohesion: 0.10
Nodes (27): _addButton(), addControls(), applyIconStyle(), _btnNameMapping(), changeActionsOfControl(), changeControlOrder(), controlExists(), copyDrawControl() (+19 more)

### Community 143 - "Admin Panel Filament Resources"
Cohesion: 0.12
Nodes (22): B(), close(), E(), F(), G(), I(), L(), N() (+14 more)

### Community 145 - "Subsystem: match()"
Cohesion: 0.12
Nodes (25): d0(), De(), E$(), f0(), getCursor(), gT(), h0(), highlight() (+17 more)

### Community 146 - "Employee & Identity Management"
Cohesion: 0.08
Nodes (24): Doc: rest-best-practices.md, Authentication and Authorization, Bad - Verbs or mixed conventions, Better:, Caching, Client caching, Conditional requests, Deep nesting (avoid) (+16 more)

### Community 147 - "Odoo ERP & Principal Integration"
Cohesion: 0.10
Nodes (8): CreateWorkLocation, EditWorkLocation, WorkLocationForm, WorkLocationsTable, Builder, UnitEnum, WorkLocationResource, Livewire\Attributes\On

### Community 148 - "Subsystem: octane.php"
Cohesion: 0.09
Nodes (22): Laravel\Octane\Contracts\OperationTerminated, Laravel\Octane\Events\RequestHandled, Laravel\Octane\Events\RequestReceived, Laravel\Octane\Events\RequestTerminated, Laravel\Octane\Events\TaskReceived, Laravel\Octane\Events\TaskTerminated, Laravel\Octane\Events\TickReceived, Laravel\Octane\Events\TickTerminated (+14 more)

### Community 149 - "Reporting & Form Engine"
Cohesion: 0.15
Nodes (6): create_presentation(), pptx, pptx_dml_color, pptx_enum_shapes, pptx_enum_text, pptx_util

### Community 150 - "Employee & Identity Management"
Cohesion: 0.18
Nodes (5): LiveChat, BackedEnum, UnitEnum, Width, ChatMessage

### Community 151 - "API & Mobile State Providers"
Cohesion: 0.11
Nodes (4): PermitController, SalesPipelineController, Illuminate\Support\Carbon, Illuminate\Support\Facades\Validator

### Community 152 - "Subsystem: att-admin-v12/package.jso"
Cohesion: 0.12
Nodes (17): devDependencies, concurrently, laravel-vite-plugin, tailwindcss, @tailwindcss/vite, vite, private, $schema (+9 more)

### Community 153 - "Subsystem: oo()"
Cohesion: 0.15
Nodes (19): co(), _cutLayer(), ds(), fo(), go(), ho(), Ks(), lo() (+11 more)

### Community 154 - "Admin Panel Filament Resources"
Cohesion: 0.11
Nodes (19): Doc: SKILL.md, Constraints, Core Rules, Data Types, Do not use this skill when, Examples, Extensions, Generated Columns (+11 more)

### Community 156 - "Employee & Identity Management"
Cohesion: 0.14
Nodes (7): {closure#1}(), PermissionMatrix, CreateRole, UnitEnum, RoleResource, RoleForm, Filament\Forms\Components\Field

### Community 157 - "Odoo ERP & Principal Integration"
Cohesion: 0.11
Nodes (18): devDependencies, axios, @babel/core, babel-loader, @babel/plugin-proposal-object-rest-spread, @babel/plugin-syntax-dynamic-import, @babel/plugin-transform-runtime, @babel/preset-env (+10 more)

### Community 158 - "API & Mobile State Providers"
Cohesion: 0.19
Nodes (11): get_local_ip(), get_session_stats(), load_config(), MonitorHandler, run(), save_config(), BaseHTTPRequestHandler, DateTime (+3 more)

### Community 160 - "Flutter Mobile UI Components"
Cohesion: 0.21
Nodes (17): areRecordsSelected(), areRecordsToggleable(), canSelectAllRecords(), deselectAllRecords(), deselectRecords(), getRecordsOnPage(), getSelectedRecordsCount(), handleCheckboxClick() (+9 more)

### Community 161 - "Reporting & Form Engine"
Cohesion: 0.14
Nodes (4): gzip, openpyxl, shutil, sqlite3

### Community 162 - "API & Mobile State Providers"
Cohesion: 0.12
Nodes (17): Doc: implementation-playbook.md, API Design Principles Implementation Playbook, Bad: Action-oriented endpoints (avoid), Clear type definitions, Consistent error responses, Core Concepts, Custom scalars, Enums for type safety (+9 more)

### Community 163 - "Flutter Mobile UI Components"
Cohesion: 0.17
Nodes (5): HolidayResource, UnitEnum, CreateHoliday, EditHoliday, ListHolidays

### Community 164 - "Subsystem: t()"
Cohesion: 0.14
Nodes (15): b(), di(), e(), g(), Ht(), i(), Ie(), Me() (+7 more)

### Community 165 - "Subsystem: color-picker.js"
Cohesion: 0.13
Nodes (4): [g](), style(), update(), [x]()

### Community 166 - "Subsystem: fn()"
Cohesion: 0.26
Nodes (16): Ae(), Ce(), De(), fn(), Ft(), ht(), ii(), Le() (+8 more)

### Community 167 - "Admin Panel Filament Resources"
Cohesion: 0.17
Nodes (16): afterAutoSkip(), Ar(), buildLookupTable(), getAllParsedValues(), getDataTimestamps(), getDecimalForValue(), _getLabelBounds(), getLabelTimestamps() (+8 more)

### Community 168 - "Subsystem: TestCase"
Cohesion: 0.16
Nodes (8): ExampleTest, TestCase, CreatesApplication, ExampleTest, TestCase, Illuminate\Contracts\Console\Kernel, Illuminate\Foundation\Testing\RefreshDatabase, Illuminate\Foundation\Testing\TestCase

### Community 169 - "Admin Panel Filament Resources"
Cohesion: 0.12
Nodes (16): Doc: implementation-playbook.md, - Hook NtQueryInformationProcess, - Patch PEB.BeingDebugged to 0, - Set IsDebuggerPresent return to 0, Anti-Debugging Techniques, Anti-Reversing Techniques, Anti-Reversing Techniques Implementation Playbook, Anti-VM Detection (+8 more)

### Community 170 - "Attendance & Schedule Subsystem"
Cohesion: 0.23
Nodes (6): AttendanceExporter, ItineraryExporter, VisitReportExporter, Filament\Actions\Exports\ExportColumn, Filament\Actions\Exports\Exporter, Filament\Actions\Exports\Models\Export

### Community 171 - "API & Mobile State Providers"
Cohesion: 0.13
Nodes (15): require, barryvdh/laravel-dompdf, dotswan/filament-map-picker, filament/filament, google/apiclient, intervention/image, laravel/framework, laravel/octane (+7 more)

### Community 172 - "Admin Panel Filament Resources"
Cohesion: 0.14
Nodes (6): html_parser, HTMLParser, SimpleParser, json, urllib_parse, urllib_request

### Community 173 - "Subsystem: .prettierrc.json"
Cohesion: 0.13
Nodes (14): arrowParens, bracketSpacing, htmlWhitespaceSensitivity, insertPragma, jsxSingleQuote, printWidth, proseWrap, quoteProps (+6 more)

### Community 174 - "Reporting & Form Engine"
Cohesion: 0.16
Nodes (6): CreateReportTemplate, {closure#1}(), ListReportTemplates, Builder, UnitEnum, ReportTemplateResource

### Community 175 - "Flutter Mobile UI Components"
Cohesion: 0.30
Nodes (14): closeDropdown(), constructor(), destroy(), focusNextOption(), focusPreviousOption(), getVisibleOptions(), handleDropdownKeydown(), handleSelectButtonKeydown() (+6 more)

### Community 176 - "Attendance & Schedule Subsystem"
Cohesion: 0.14
Nodes (14): Doc: ROADMAP.md, Master Implementation Plan (Roadmap Lanjutan), ✅ Tahap 10: Penguncian Parameter Odoo Sync (NIK + , ✅ Tahap 11: Pengikatan Department ke Prinsiple & A, ✅ Tahap 1: Penyelarasan Antarmuka (UI Overhaul) & , ✅ Tahap 2: Manajemen Target HK & Payslip (SELESAI), ✅ Tahap 3: Laporan Sales (OOS, Plano, Promo) & Pem, ✅ Tahap 4: Laporan Tren Analitik (Web Admin) (SELE (+6 more)

### Community 177 - "Attendance & Schedule Subsystem"
Cohesion: 0.14
Nodes (14): Doc: ROADMAP.md, Master Implementation Plan (Roadmap Lanjutan), ✅ Tahap 10: Penguncian Parameter Odoo Sync (NIK + , ✅ Tahap 11: Pengikatan Department ke Prinsiple & A, ✅ Tahap 1: Penyelarasan Antarmuka (UI Overhaul) & , ✅ Tahap 2: Manajemen Target HK & Payslip (SELESAI), ✅ Tahap 3: Laporan Sales (OOS, Plano, Promo) & Pem, ✅ Tahap 4: Laporan Tren Analitik (Web Admin) (SELE (+6 more)

### Community 179 - "Reporting & Form Engine"
Cohesion: 0.35
Nodes (8): {closure#150}(), {closure#151}(), {closure#153}(), {closure#156}(), {closure#161}(), {closure#165}(), {closure#169}(), {closure#91}()

### Community 180 - "Flutter Mobile UI Components"
Cohesion: 0.28
Nodes (13): addBadgesForSelectedOptions(), addSingleBadge(), addSingleSelectionDisplay(), createBadgeElement(), createRemoveButton(), getLabelForSingleSelection(), getLabelsForMultipleSelection(), getSelectedOptionLabel() (+5 more)

### Community 181 - "Subsystem: renderOptions()"
Cohesion: 0.37
Nodes (13): createOptionElement(), deferPositionDropdown(), filterOptions(), handleSearch(), hideLoadingState(), openDropdown(), populateLabelRepositoryFromOptions(), positionDropdown() (+5 more)

### Community 182 - "Subsystem: e()"
Cohesion: 0.21
Nodes (13): A(), e(), ue(), vi(), c(), X(), a(), toggleFiltersDropdown() (+5 more)

### Community 183 - "Employee & Identity Management"
Cohesion: 0.15
Nodes (13): Doc: implementation-playbook.md, Bad: Executes N+1 queries, Core Concepts, Do not use this skill when, Good: Single query with JOIN or batch load, Group orders by user_id, Instructions, Optimization Patterns (+5 more)

### Community 184 - "Attendance & Schedule Subsystem"
Cohesion: 0.17
Nodes (11): addAllEmployeesFromArea, goToStep1, goToStep2, nextTablePage({{ $pagination[, previousTablePage, removeAllEmployees, removeEmployee({{ $emp->id }}), saveAndGenerateSchedule (+3 more)

### Community 185 - "Subsystem: ResetPasswordMail"
Cohesion: 0.21
Nodes (7): {closure#19}(), ResetPasswordMail, Illuminate\Contracts\Queue\ShouldQueue, Illuminate\Mail\Mailable, Illuminate\Mail\Mailables\Attachment, Illuminate\Mail\Mailables\Content, Illuminate\Mail\Mailables\Envelope

### Community 186 - "Admin Panel Filament Resources"
Cohesion: 0.17
Nodes (11): autoload-dev, psr-4, description, keywords, license, minimum-stability, name, prefer-stable (+3 more)

### Community 187 - "Subsystem: date-time-picker.js"
Cohesion: 0.29
Nodes (7): d(), e(), i(), m(), r(), s(), t()

### Community 190 - "Admin Panel Filament Resources"
Cohesion: 0.17
Nodes (12): Doc: SKILL.md, Configuration Options, Do not use this skill when, Instructions, Phase 1: Performance Profiling & Baseline, Phase 2: Database & Backend Optimization, Phase 3: Frontend & CDN Optimization, Phase 4: Load Testing & Validation (+4 more)

### Community 191 - "Subsystem: Doc: SKILL.md"
Cohesion: 0.17
Nodes (12): Doc: SKILL.md, Configuration Options, Coordination Notes, Do not use this skill when, Instructions, Phase 1: Comprehensive Security Assessment, Phase 2: Vulnerability Remediation, Phase 3: Security Controls Implementation (+4 more)

### Community 192 - "Attendance & Schedule Subsystem"
Cohesion: 0.17
Nodes (12): Doc: PANDUAN_SETTING_AAPANEL_PRODUCTION.md, Langkah 1: Instalasi aaPanel pada VPS Ubuntu 22.04, Langkah 2: Instalasi Komponen Environment (App Sto, Langkah 3: Konfigurasi & Ekstensi PHP 8.2, Langkah 4: Setup Website & Domain di aaPanel, Langkah 5: Clone Project & Konfigurasi Environment, Langkah 6: Setting Supervisor (Antrean Job Queue) , Langkah 7: Setting Cron Job Otomatis (Odoo Sync &  (+4 more)

### Community 193 - "Reporting & Form Engine"
Cohesion: 0.17
Nodes (12): Doc: pubspec.yaml, Dependencies specify other packages that your pack, For information on the generic Dart part of this f, The following line prevents the package from being, The following section is specific to Flutter packa, To automatically upgrade your package dependencies, consider running `flutter pub upgrade --major-vers, dependencies can be manually updated by changing t (+4 more)

### Community 194 - "Subsystem: _calcLatLngDistances()"
Cohesion: 0.20
Nodes (10): _calcClosestLayer(), _calcClosestLayers(), _calcLatLngDistances(), _calcLayerDistances(), _checkPrioritiySnapping(), _getClosestLayerByPriority(), _getClosestPointOnSegment(), _getDistance() (+2 more)

### Community 195 - "Flutter Mobile UI Components"
Cohesion: 0.18
Nodes (3): Bn(), ji(), Xc()

### Community 196 - "Subsystem: webpack.mix.js"
Cohesion: 0.18
Nodes (9): { EnvironmentPlugin }, glob, mix, path, sassOptions, ref_glob, laravel-mix, ref_path (+1 more)

### Community 197 - "Employee & Identity Management"
Cohesion: 0.18
Nodes (11): Doc: graphql-schema-design.md, Field Design, GraphQL Schema Design Patterns, Mutation Design Patterns, Pagination Patterns, Query example, Schema Organization, Type Design Patterns (+3 more)

### Community 198 - "Subsystem: Doc: SKILL.md"
Cohesion: 0.18
Nodes (11): Doc: SKILL.md, Behavioral Traits, Capabilities, Do not use this skill when, Example Interactions, Instructions, Knowledge Base, Purpose (+3 more)

### Community 199 - "Subsystem: Doc: SKILL.md"
Cohesion: 0.18
Nodes (11): Doc: SKILL.md, Behavioral Traits, Capabilities, Do not use this skill when, Example Interactions, Instructions, Knowledge Base, Purpose (+3 more)

### Community 200 - "Attendance & Schedule Subsystem"
Cohesion: 0.18
Nodes (11): Doc: PANDUAN_SETUP_3_SERVER_PRODUCTION_ESA.md, PANDUAN LENGKAP SETTING & DEPLOYMENT 3 SERVER PROD, Sistem Presensi Mobile & Pelaporan Lapangan ESA Gr, ⏰ LANGKAH 7: SETTING CRON JOB OTOMATIS, ⚙️ LANGKAH 2: INSTALASI AAPANEL PADA 3 SERVER, ⚡ LANGKAH 6: SETTING SUPERVISOR (QUEUE WORKER REDI, 🌐 LANGKAH 1: KONFIGURASI DNS (HANYA DI 1 DASHBOARD, 🌐 LANGKAH 4: PEMBUATAN WEBSITE & SSL DI MASING-MAS (+3 more)

### Community 201 - "Subsystem: MessageSent"
Cohesion: 0.29
Nodes (7): MessageSent, Illuminate\Broadcasting\Channel, Illuminate\Broadcasting\InteractsWithSockets, Illuminate\Broadcasting\PrivateChannel, Illuminate\Contracts\Broadcasting\ShouldBroadcastNow, Illuminate\Foundation\Events\Dispatchable, Illuminate\Queue\SerializesModels

### Community 202 - "Subsystem: Fe()"
Cohesion: 0.20
Nodes (10): Ce(), De(), Dt(), Fe(), He(), ir(), Mt(), nr() (+2 more)

### Community 203 - "Attendance & Schedule Subsystem"
Cohesion: 0.20
Nodes (9): mountAction(, nextPage({{ $pagination[, previousPage, closeWorkingGroupModal, deleteWorkingGroup({{ $wg->id }}), regenerateWorkingGroup({{ $this->viewingWorkingGroup->id }}), regenerateWorkingGroup({{ $wg->id }}), setActiveTab( (+1 more)

### Community 204 - "Reporting & Form Engine"
Cohesion: 0.20
Nodes (9): portal.partials.cbp_dashboard, portal.partials.customer_database_dashboard, portal.partials.daily_maintenance_dashboard, portal.partials.offtake_dashboard, portal.partials.oos_dashboard, portal.partials.stock_dashboard, portal.partials.wings_mbr_dashboard, portal.partials.wings_mbr_freetaste_dashboard (+1 more)

### Community 205 - "Admin Panel Filament Resources"
Cohesion: 0.20
Nodes (10): Doc: SKILL.md, Context, Do not use this skill when, Instructions, Output Format, Refactor and Clean Code, Requirements, Resources (+2 more)

### Community 206 - "Subsystem: Doc: implementation-playb"
Cohesion: 0.20
Nodes (10): Doc: implementation-playbook.md, AFTER: Each class has one responsibility, AFTER: Open for extension, closed for modification, After, BEFORE: 500-line monolithic file, BEFORE: Modification required for new discount typ, BEFORE: Multiple responsibilities in one class, Before (+2 more)

### Community 207 - "Subsystem: Doc: SKILL.md"
Cohesion: 0.20
Nodes (10): Doc: SKILL.md, Behavioral Traits, Capabilities, Do not use this skill when, Example Interactions, Instructions, Knowledge Base, Purpose (+2 more)

### Community 208 - "Subsystem: Doc: SKILL.md"
Cohesion: 0.20
Nodes (10): Doc: SKILL.md, Behavioral Traits, Capabilities, Do not use this skill when, Example Interactions, Instructions, Knowledge Base, Purpose (+2 more)

### Community 209 - "Subsystem: Doc: PANDUAN_MIGRASI_SWIT"
Cohesion: 0.20
Nodes (10): Doc: PANDUAN_MIGRASI_SWITCH_DOMAIN_UTAMA_ESA.md, Zero-Downtime Transition Guide untuk 3 Server Prod, ⚙️ 3. LANGKAH 2: TAMBAHKAN DOMAIN DI AAPANEL MASIN, ✅ 6. LANGKAH 5: PENGUJIAN & VERIFIKASI AKHIR, 🌐 2. LANGKAH 1: SETTING DNS RECORD DI DASHBOARD `e, 🌐 PANDUAN SWITCH / MIGRASI KE DOMAIN UTAMA (esa-so, 📋 1. RINGKASAN PEMETAAN IP SERVER PRODUCTION, 📝 5. LANGKAH 4: UPDATE `APP_URL` DI FILE `.ENV` &  (+2 more)

### Community 210 - "Odoo ERP & Principal Integration"
Cohesion: 0.20
Nodes (10): Doc: PLAN_REVERSE_SYNC_ODOO.md, 1. Arsitektur & Gambaran Umum, 2. Pemetaan Data & Model Odoo, 3. Penyesuaian Skema Database Lokal, 4. Standar Penanganan Waktu & Timezone (Wajib UTC), 5. Contoh Rancangan Method di `OdooSyncService.php, 6. Pilihan Pemicu Sinkronisasi (*Trigger Mechanism, 7. Prasyarat & Keamanan di Sisi Odoo (+2 more)

### Community 211 - "Reporting & Form Engine"
Cohesion: 0.20
Nodes (10): Doc: analysis_options.yaml, Additional information about this file can be foun, IDEs (https://dart.dev/tools#ides-and-editors). Th, The following line activates a set of recommended , The issues identified by the analyzer are surfaced, This file configures the analyzer, which staticall, check for errors, warnings, and lints., https://dart.dev/guides/language/analysis-options (+2 more)

### Community 212 - "Visit & Itinerary Management"
Cohesion: 0.22
Nodes (3): Builder, UnitEnum, VisitReportResource

### Community 213 - "Flutter Mobile UI Components"
Cohesion: 0.22
Nodes (3): Builder, UnitEnum, WorkTargetResource

### Community 215 - "Subsystem: PermitStatusUpdated"
Cohesion: 0.31
Nodes (4): PermitStatusUpdated, Illuminate\Bus\Queueable, Illuminate\Notifications\Messages\MailMessage, Illuminate\Notifications\Notification

### Community 216 - "Subsystem: att-admin-v12/bootstrap/a"
Cohesion: 0.28
Nodes (7): {closure#1}(), {closure#2}(), {closure#3}(), SortDirection, Illuminate\Foundation\Application, Illuminate\Foundation\Configuration\Exceptions, Illuminate\Foundation\Configuration\Middleware

### Community 217 - "Subsystem: scripts"
Cohesion: 0.22
Nodes (9): scripts, dev, post-autoload-dump, post-create-project-cmd, post-root-package-install, post-update-cmd, pre-package-uninstall, setup (+1 more)

### Community 219 - "Employee & Identity Management"
Cohesion: 0.22
Nodes (8): _partials/_modals/modal-add-new-address, _partials/_modals/modal-add-new-cc, _partials/_modals/modal-edit-user, _partials/_modals/modal-enable-otp, _partials/_modals/modal-create-app, _partials/_modals/modal-refer-earn, _partials/_modals/modal-share-project, _partials/_modals/modal-two-factor-auth

### Community 220 - "Admin Panel Filament Resources"
Cohesion: 0.22
Nodes (9): Doc: SKILL.md, Context, Do not use this skill when, Instructions, Output Format, Requirements, Resources, SQL Database Migration Strategy and Implementation (+1 more)

### Community 221 - "Admin Panel Filament Resources"
Cohesion: 0.22
Nodes (9): Doc: SKILL.md, Context, Dependency Audit and Security Analysis, Do not use this skill when, Instructions, Requirements, Resources, Safety (+1 more)

### Community 222 - "Subsystem: require-dev"
Cohesion: 0.25
Nodes (8): require-dev, fakerphp/faker, laravel/pail, laravel/pao, laravel/pint, mockery/mockery, nunomaduro/collision, phpunit/phpunit

### Community 223 - "Subsystem: jt()"
Cohesion: 0.29
Nodes (8): constructor(), define(), getExtension(), _getTestState(), getType(), registerListeners(), jt(), St()

### Community 224 - "API & Mobile State Providers"
Cohesion: 0.46
Nodes (8): addInner(), bu(), dy(), mapInner(), Mo(), wu(), yu(), Zt()

### Community 225 - "Attendance & Schedule Subsystem"
Cohesion: 0.25
Nodes (7): closeDetailModal, deleteItinerary({{ $selectedItinerary[, nextMonth, openAddModal(, openDetailModal({{ $sch[, prevMonth, today

### Community 226 - "Flutter Mobile UI Components"
Cohesion: 0.25
Nodes (3): Helpers, Horizontal, Config

### Community 227 - "Employee & Identity Management"
Cohesion: 0.29
Nodes (5): EventServiceProvider, Illuminate\Auth\Events\Registered, Illuminate\Auth\Listeners\SendEmailVerificationNotification, Illuminate\Foundation\Support\Providers\EventServiceProvider, Illuminate\Support\Facades\Event

### Community 228 - "Subsystem: scripts"
Cohesion: 0.25
Nodes (8): scripts, dev, development, hot, prod, production, watch, watch-poll

### Community 230 - "Subsystem: Doc: README.md"
Cohesion: 0.25
Nodes (8): Doc: README.md, About Laravel, Agentic Development, Code of Conduct, Contributing, Learning Laravel, License, Security Vulnerabilities

### Community 231 - "Subsystem: Doc: README.md"
Cohesion: 0.25
Nodes (8): Doc: README.md, About Laravel, Code of Conduct, Contributing, Laravel Sponsors, Learning Laravel, License, Security Vulnerabilities

### Community 232 - "Attendance & Schedule Subsystem"
Cohesion: 0.29
Nodes (3): AttendanceImportTemplateExport, {closure#6}(), {closure#12}()

### Community 234 - "API & Mobile State Providers"
Cohesion: 0.29
Nodes (7): pestphp/pest-plugin, php-http/discovery, config, allow-plugins, optimize-autoloader, preferred-install, sort-packages

### Community 235 - "Odoo ERP & Principal Integration"
Cohesion: 0.62
Nodes (6): closeModal(), generateModalId(), getActionNestingIndexFromModalId(), init(), openModal(), syncActionModals()

### Community 236 - "Subsystem: PHPUnit\Framework\TestCas"
Cohesion: 0.38
Nodes (3): ExampleTest, ExampleTest, PHPUnit\Framework\TestCase

### Community 237 - "Subsystem: app-kanban.js"
Cohesion: 0.38
Nodes (4): renderAvatar(), renderDropdown(), renderFooter(), renderHeader()

### Community 238 - "Subsystem: payslip.dart"
Cohesion: 0.29
Nodes (6): createdAt, fileUrl, fromJson, id, monthYear, Payslip

### Community 239 - "Employee & Identity Management"
Cohesion: 0.29
Nodes (7): Doc: SKILL.md, Authentication & Authorization Implementation Patt, Do not use this skill when, Instructions, Resources, Safety, Use this skill when

### Community 240 - "Admin Panel Filament Resources"
Cohesion: 0.29
Nodes (7): Doc: SKILL.md, Bash Defensive Patterns, Do not use this skill when, Instructions, Resources, Safety, Use this skill when

### Community 241 - "Subsystem: Doc: implementation-playb"
Cohesion: 0.29
Nodes (7): Doc: implementation-playbook.md, Context, Do not use this skill when, Instructions, Requirements, SQL Database Migration Strategy and Implementation, Use this skill when

### Community 242 - "Subsystem: Doc: SKILL.md"
Cohesion: 0.29
Nodes (7): Doc: SKILL.md, Approach, Do not use this skill when, Focus Areas, Instructions, Output, Use this skill when

### Community 243 - "Subsystem: Doc: SKILL.md"
Cohesion: 0.29
Nodes (7): Doc: SKILL.md, Automated Unit Test Generation, Context, Do not use this skill when, Instructions, Requirements, Use this skill when

### Community 244 - "Attendance & Schedule Subsystem"
Cohesion: 0.29
Nodes (7): Doc: ARSITEKTUR_SINKRONISASI_CROSS_ENTITY_ESA.md, 1. Latar Belakang & Tantangan Hierarki Lintas Enti, 2. Komparasi 3 Opsi Arsitektur Server & Sinkronisa, 3. Strategi Sinkronisasi Master Data dari Odoo, 4. Rekomendasi Implementasi Bertahap (Roadmap), ARSITEKTUR INFRASTRUKTUR & STRATEGI SINKRONISASI C, Sistem Presensi & Pelaporan Lapangan ESA Groups (2

### Community 245 - "Odoo ERP & Principal Integration"
Cohesion: 0.29
Nodes (7): Doc: PANDUAN_AUTO_DEPLOY_3_SERVER.md, Server Development: `appsend.my.id` $\rightarrow$ , 📋 1. Alur & Konfigurasi Server, 🔑 2. LANGKAH PERSIAPAN (HANYA DILAKUKAN 1 KALI SAJ, 🚀 3. CARA DEPLOY KE SELURUH SERVER (SETIAP KALI SE, 🚀 PANDUAN OTOMATISASI DEPLOYMENT 3 SERVER PRODUCTI, 🛠️ 4. PENYESUAIAN KOLOM PRINSIPLE KLIEN (UPDATE TE

### Community 247 - "Employee & Identity Management"
Cohesion: 0.53
Nodes (4): LoginResponse, Filament\Auth\Http\Responses\Contracts\LoginResponse, Illuminate\Http\RedirectResponse, Livewire\Features\SupportRedirects\Redirector

### Community 248 - "Subsystem: att-admin-v12/config/logg"
Cohesion: 0.47
Nodes (4): Monolog\Handler\NullHandler, Monolog\Handler\StreamHandler, Monolog\Handler\SyslogUdpHandler, Monolog\Processor\PsrLogMessageProcessor

### Community 250 - "Attendance & Schedule Subsystem"
Cohesion: 0.47
Nodes (3): Kernel, Illuminate\Console\Scheduling\Schedule, Illuminate\Foundation\Console\Kernel

### Community 251 - "Subsystem: Handler.php"
Cohesion: 0.47
Nodes (4): {closure#1}(), Handler, Illuminate\Foundation\Exceptions\Handler, Throwable

### Community 252 - "Employee & Identity Management"
Cohesion: 0.60
Nodes (5): Controller, Illuminate\Foundation\Auth\Access\AuthorizesRequests, Illuminate\Foundation\Bus\DispatchesJobs, Illuminate\Foundation\Validation\ValidatesRequests, Illuminate\Routing\Controller

### Community 253 - "Employee & Identity Management"
Cohesion: 0.33
Nodes (5): _partials/_modals/modal-add-new-address, _partials/_modals/modal-add-new-cc, _partials/_modals/modal-edit-cc, _partials/_modals/modal-edit-user, _partials/_modals/modal-upgrade-plan

### Community 254 - "Subsystem: sys"
Cohesion: 0.33
Nodes (3): sys, xml_etree_elementtree, zipfile

### Community 255 - "Admin Panel Filament Resources"
Cohesion: 0.33
Nodes (6): Doc: SKILL.md, Do not use this skill when, Instructions, Resources, Safety, Use this skill when

### Community 256 - "API & Mobile State Providers"
Cohesion: 0.33
Nodes (6): Doc: SKILL.md, API Design Principles, Do not use this skill when, Instructions, Resources, Use this skill when

### Community 257 - "Employee & Identity Management"
Cohesion: 0.33
Nodes (6): Doc: implementation-playbook.md, Authentication and Authorization Implementation Pa, Core Concepts, JWT Authentication, OAuth2 / Social Login, Session-Based Authentication

### Community 258 - "Admin Panel Filament Resources"
Cohesion: 0.33
Nodes (6): Doc: SKILL.md, Do not use this skill when, Instructions, Resources, SQL Optimization Patterns, Use this skill when

### Community 259 - "Attendance & Schedule Subsystem"
Cohesion: 0.33
Nodes (6): Doc: TODO_LIVE_TRACKING_IMPROVEMENT.md, Rencana Perbaikan Live Tracking Mobile (Flutter) -, 📌 1. Pemicu Tracking pada Presensi Kunjungan (*Vis, 📌 2. Memastikan Status Foreground Service Terkunci, 📌 3. Optimalisasi Pengambilan GPS Saat Layar HP Te, 📌 4. Edukasi Izin Notifikasi & Optimasi Baterai Ve

### Community 261 - "Subsystem: psr-4"
Cohesion: 0.40
Nodes (5): autoload, psr-4, App\\, Database\\Factories\\, Database\\Seeders\\

### Community 263 - "Subsystem: att-admin-v12/deploy.sh"
Cohesion: 0.70
Nodes (4): deploy_local_dev(), deploy_three_prod_servers(), setup_ssh_pairing(), deploy.sh script

### Community 264 - "Employee & Identity Management"
Cohesion: 0.40
Nodes (4): closeNewChatModal, openNewChatModal, selectConversation({{ $conv->id }}), startConversationWithEmployee({{ $emp->id }})

### Community 265 - "Reporting & Form Engine"
Cohesion: 0.40
Nodes (4): closeLogDetail, $refresh, setToday, showLogDetail({{ $log->id }})

### Community 266 - "Employee & Identity Management"
Cohesion: 0.50
Nodes (3): AuthServiceProvider, Illuminate\Foundation\Support\Providers\AuthServiceProvider, Illuminate\Support\Facades\Gate

### Community 267 - "Subsystem: overrides"
Cohesion: 0.40
Nodes (5): overrides, autoprefixer, prop-types, webpack, webpack-cli

### Community 268 - "Subsystem: resolutions"
Cohesion: 0.40
Nodes (5): resolutions, autoprefixer, prop-types, webpack, webpack-cli

### Community 269 - "Subsystem: contentNavbarLayout.blade"
Cohesion: 0.40
Nodes (4): layouts/sections/footer/footer, layouts/sections/navbar/navbar, _partials/_modals/modal-pricing, layouts/sections/menu/verticalMenu

### Community 270 - "Subsystem: horizontalLayout.blade.ph"
Cohesion: 0.40
Nodes (4): layouts/sections/footer/footer, layouts/sections/navbar/navbar, _partials/_modals/modal-pricing, layouts/sections/menu/horizontalMenu

### Community 272 - "Subsystem: deploy.sh"
Cohesion: 0.70
Nodes (4): deploy_local_dev(), deploy_three_prod_servers(), setup_ssh_pairing(), deploy.sh script

### Community 273 - "Attendance & Schedule Subsystem"
Cohesion: 0.40
Nodes (5): Doc: rancangan_sistem_attendance_laravel_mysql_android.md, 1. Tujuan Sistem, 2. Aktor / Role Pengguna, 3. Modul Sistem, Rancangan Sistem Aplikasi Attendance

### Community 276 - "Attendance & Schedule Subsystem"
Cohesion: 0.50
Nodes (3): mountAction(, nextPage({{ $pagination[, previousPage

### Community 279 - "Subsystem: app-calendar-events.js"
Cohesion: 0.50
Nodes (3): date, events, nextDay

### Community 280 - "Reporting & Form Engine"
Cohesion: 0.50
Nodes (3): assetsPath, config, templateName

### Community 282 - "Subsystem: app-invoice-edit.blade.ph"
Cohesion: 0.50
Nodes (3): _partials.macros, _partials/_offcanvas/offcanvas-add-payment, _partials/_offcanvas/offcanvas-send-invoice

### Community 283 - "Flutter Mobile UI Components"
Cohesion: 0.50
Nodes (3): _partials.macros, _partials/_offcanvas/offcanvas-add-payment, _partials/_offcanvas/offcanvas-send-invoice

### Community 284 - "Employee & Identity Management"
Cohesion: 0.50
Nodes (3): _partials/_modals/modal-edit-user, _partials/_modals/modal-enable-otp, _partials/_modals/modal-upgrade-plan

### Community 285 - "Subsystem: commonMaster.blade.php"
Cohesion: 0.50
Nodes (3): layouts/sections/scripts, layouts/sections/scriptsIncludes, layouts/sections/styles

### Community 286 - "Flutter Mobile UI Components"
Cohesion: 0.50
Nodes (4): Doc: api-design-checklist.md, API Design Checklist, GraphQL-Specific Checks, Pre-Implementation Review

### Community 287 - "Attendance & Schedule Subsystem"
Cohesion: 0.50
Nodes (4): Doc: README.md, Catatan Penting Pengembangan (Update Terakhir: 4 A, Proyek Aplikasi Absensi (att-mobile & att-admin), Riwayat Update (Changelog)

### Community 288 - "Reporting & Form Engine"
Cohesion: 0.50
Nodes (4): Doc: Spesifikasi_Fitur_Sales_Reporting-v2.md, Daftar Fitur Utama, Latar Belakang, Spesifikasi Fitur: Modul Sales Reporting (Dengan I

### Community 290 - "Subsystem: extra"
Cohesion: 0.67
Nodes (3): extra, laravel, dont-discover

### Community 291 - "Subsystem: pt()"
Cohesion: 0.67
Nodes (3): H(), ji(), pt()

### Community 365 - "Subsystem: Doc: implementation-playb"
Cohesion: 0.67
Nodes (3): Doc: implementation-playbook.md, Dependency Audit and Security Analysis Implementat, Instructions

### Community 366 - "Subsystem: Doc: README.md"
Cohesion: 0.67
Nodes (3): Doc: README.md, Getting Started, att_mobile

### Community 367 - "Attendance & Schedule Subsystem"
Cohesion: 0.67
Nodes (3): Doc: README_MOBILE_INTEGRATION.md, PANDUAN INTEGRASI MOBILE CLIENT (MULTI-SERVER ROUT, Project Aplikasi Presensi & Pelaporan ESA Groups (

## Knowledge Gaps
- **2142 isolated node(s):** `SortDirection`, `$schema`, `name`, `type`, `description` (+2137 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 5427 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **519 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `BlastInfo` connect `Visit & Itinerary Management` to `Attendance & Schedule Subsystem`, `Flutter Mobile UI Components`, `Visit & Itinerary Management`?**
  _High betweenness centrality (0.032) - this node is a cross-community bridge._
- **Why does `Controller` connect `Employee & Identity Management` to `API & Mobile State Providers`, `Odoo ERP & Principal Integration`, `Odoo ERP & Principal Integration`, `Visit & Itinerary Management`, `Attendance & Schedule Subsystem`, `Attendance & Schedule Subsystem`, `API & Mobile State Providers`, `API & Mobile State Providers`, `Employee & Identity Management`, `Attendance & Schedule Subsystem`, `Subsystem: AccessPermission.php`, `Subsystem: InvoiceAdd.php`, `Subsystem: InvoiceList.php`, `Subsystem: InvoicePrint.php`, `Employee & Identity Management`, `Employee & Identity Management`, `Employee & Identity Management`, `Subsystem: ForgotPasswordCover.php`, `Employee & Identity Management`, `Subsystem: RegisterBasic.php`, `Subsystem: RegisterCover.php`, `Subsystem: TwoStepsCover.php`, `Subsystem: VerifyEmailBasic.php`, `Employee & Identity Management`, `Subsystem: VerifyEmailCover.php`, `Flutter Mobile UI Components`, `Subsystem: ApexCharts.php`, `Subsystem: Analytics.php`, `Subsystem: Crm.php`, `Subsystem: DragAndDrop.php`, `Subsystem: Misc.php`, `Subsystem: StarRatings.php`, `Subsystem: Tour.php`, `Flutter Mobile UI Components`, `Subsystem: InputGroups.php`, `Subsystem: Picker.php`, `Subsystem: Selects.php`, `Subsystem: Sliders.php`, `Subsystem: Switches.php`, `Employee & Identity Management`, `Admin Panel Filament Resources`, `Subsystem: Numbered.php`, `Admin Panel Filament Resources`, `Subsystem: Blank.php`, `Subsystem: Container.php`, `Subsystem: ContentNavSidebar.php`, `Subsystem: WithoutMenu.php`, `Subsystem: AccountSettingsAccount.ph`, `Subsystem: AccountSettingsNotificati`, `Subsystem: AccountSettingsSecurity.p`, `Subsystem: Faq.php`, `Subsystem: HelpCenterCategories.php`, `Subsystem: MiscError.php`, `Employee & Identity Management`, `Reporting & Form Engine`, `Subsystem: Pricing.php`, `Employee & Identity Management`, `Employee & Identity Management`, `Subsystem: Basic.php`, `Employee & Identity Management`, `Admin Panel Filament Resources`, `Subsystem: Accordion.php`, `Subsystem: Collapse.php`, `Subsystem: Dropdowns.php`, `Reporting & Form Engine`, `Subsystem: Offcanvas.php`, `Subsystem: TabsPills.php`, `Subsystem: Toasts.php`, `Subsystem: TooltipsPopovers.php`, `Subsystem: Typography.php`, `Subsystem: PropertyListing.php`, `Flutter Mobile UI Components`, `Odoo ERP & Principal Integration`, `Visit & Itinerary Management`, `API & Mobile State Providers`?**
  _High betweenness centrality (0.028) - this node is a cross-community bridge._
- **Why does `Principal` connect `Subsystem: ReportTemplate` to `Odoo ERP & Principal Integration`, `Employee & Identity Management`, `Odoo ERP & Principal Integration`, `Visit & Itinerary Management`, `Odoo ERP & Principal Integration`, `Attendance & Schedule Subsystem`, `Attendance & Schedule Subsystem`, `Attendance & Schedule Subsystem`, `Admin Panel Filament Resources`, `Attendance & Schedule Subsystem`, `Attendance & Schedule Subsystem`, `Attendance & Schedule Subsystem`, `Reporting & Form Engine`, `Employee & Identity Management`, `Odoo ERP & Principal Integration`, `Odoo ERP & Principal Integration`, `Reporting & Form Engine`, `Reporting & Form Engine`, `Attendance & Schedule Subsystem`, `Attendance & Schedule Subsystem`, `API & Mobile State Providers`?**
  _High betweenness centrality (0.025) - this node is a cross-community bridge._
- **What connects `SortDirection`, `$schema`, `name` to the rest of the system?**
  _2142 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Employee & Identity Management` be split into smaller, more focused modules?**
  _Cohesion score 0.005530688517260317 - nodes in this community are weakly interconnected._
- **Should `Employee & Identity Management` be split into smaller, more focused modules?**
  _Cohesion score 0.009372979961215255 - nodes in this community are weakly interconnected._
- **Should `Subsystem: ReportTemplate` be split into smaller, more focused modules?**
  _Cohesion score 0.016938572847212372 - nodes in this community are weakly interconnected._