...
class EmployeeScheduleRangeTemplateExport extends DefaultValueBinder implements FromArray, WithHeadings, WithStyles, ShouldAutoSize, WithCustomValueBinder
{
    ...
    public function array(): array
    {
        ...
        $employeeQuery = Employee::where('is_active', 1)->with('branch');
        if (auth()->check()) {
        ...
        $sampleShiftName = $sampleShift ? ($sampleShift->code ?: $sampleShift->name) : 'OFFICE';
        
        $sampleLocation = WorkLocation::first();
        $sampleLocationName = $sampleLocation ? $sampleLocation->name : 'OUTLET UTAMA';
        ...
        if ($sampleEmployees->isNotEmpty()) {
            foreach ($sampleEmployees as $index => $emp) {
                $rows[] = [
                    ...
                    'lokasi_kerja' => $emp->branch?->name ?? $sampleLocationName,
                ];
            }
        } else {
            $rows[] = [
