<?php
$dsn = "pgsql:host=127.0.0.1;port=5432;dbname=attendance_pg;";
try {
    ...
    $stmt = $pdo->prepare("SELECT id, employee_no, first_name, employment_status, is_active, deleted_at FROM employees WHERE first_name ILIKE '%jamil%' ORDER BY id");
    $stmt->execute();
    $rows2 = $stmt->fetchAll(PDO::FETCH_ASSOC);
    print_r($rows2);
    
    // Let's get the active employee_id for Jamil
    $activeId = null;
    foreach ($rows2 as $r) {
        if ($r['employment_status'] === 'active' && $r['is_active'] && !$r['deleted_at']) {
            $activeId = $r['id'];
        }
    }
