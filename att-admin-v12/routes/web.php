...
// Web Cron Endpoint for Odoo Sync (Protected with Secret Token)
Route::get('/cron/odoo-sync', function () {
    $secretToken = getenv('DEPLOY_SECRET_TOKEN') ?: 'dgsoft_rahasia_123';
    $inputToken = (string)(request('token') ?? request()->header('X-Cron-Token') ?? '');

    if (empty($inputToken) || !hash_equals($secretToken, $inputToken)) {
        abort(403, 'Akses Ditolak: Token cron tidak valid.');
    }
