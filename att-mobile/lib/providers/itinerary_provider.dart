...
class ItineraryProvider with ChangeNotifier {
  ...
  Future<void> fetchPrincipals(AuthProvider authProvider) async {
    _setAuthToken(authProvider);
    _isLoading = true;
    _error = '';
    notifyListeners();
    ...
    try {
      final response = await _dio.get('/itineraries/principals');
      
      if (response.statusCode == 200 && response.data['status'] == 'success') {
        _principals = response.data['data'] ?? [];
      } else {
        _error = response.data['message'] ?? 'Failed to fetch principals';
      }
    } on DioException catch (e) {
      _error = e.response?.data['message'] ?? e.message ?? 'Unknown error occurred';
    } catch (e) {
